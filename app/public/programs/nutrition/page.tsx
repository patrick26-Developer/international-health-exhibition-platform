...
import { Apple, Heart, Target, Users, Award, ArrowRight, CheckCircle2, BookOpen, Leaf, Droplets, Zap, Shield, ChevronLeft, ChevronRight, Pause, Play } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { motion, AnimatePresence } from 'framer-motion';
import Link from 'next/link';
import Image from 'next/image';

// Images pour le carrousel Nutrition
const nutritionHeroImages = [
  {
    id: 1,
    src: '/images/Alimentation/legumes-de-fruits-et-de-cereales.jpg',
    alt: { 
      fr: 'Variété de légumes, fruits et céréales', 
      en: 'Variety of vegetables, fruits and cereals' 
    },
    title: { 
      fr: 'Fibres & Vitalité', 
      en: 'Fiber & Vitality' 
  ...
  {
    id: 2,
    src: '/images/ACIDES-GRAS-OMÉGA-3/poisson.jpg',
    alt: { 
      fr: 'Poisson riche en oméga-3', 
      en: 'Fish rich in omega-3' 
    },
    title: { 
      fr: 'Oméga-3 Santé', 
      en: 'Omega-3 Health' 
  ...
  {
    id: 3,
    src: '/images/ANTIOXYDANTS/complet-thé.jpeg',
    alt: { 
      fr: 'Thé et aliments riches en antioxydants', 
      en: 'Tea and antioxidant-rich foods' 
    },
    title: { 
      fr: 'Antioxydants', 
      en: 'Antioxidants' 
  ...
  {
    id: 4,
    src: '/images/PROTÉINES-DE-QUALITÉ/Légumineuses-poissons-œufs-produits-laitiers.jpg',
    alt: { 
      fr: 'Légumineuses, poissons, œufs et produits laitiers', 
      en: 'Legumes, fish, eggs and dairy products' 
    },
    title: { 
      fr: 'Protéines Qualité', 
      en: 'Quality Proteins' 
    }
  }
];

// Composant HeroCarousel pour Nutrition
function NutritionHeroCarousel({ language }: { language: 'fr' | 'en' }) {
  ...
  const goToNext = () => {
    setCurrentIndex(prev => (prev + 1) % nutritionHeroImages.length);
    resetTimer();
  };

  const goToPrev = () => {
    setCurrentIndex(prev => (prev - 1 + nutritionHeroImages.length) % nutritionHeroImages.length);
    resetTimer();
  };
  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <Image
            src={nutritionHeroImages[currentIndex].src}
            alt={nutritionHeroImages[currentIndex].alt[language]}
            fill
          ...
          <div className="absolute bottom-32 left-10 z-20">
            ...
            >
              ...
              <span className="text-white text-sm font-medium">
                {nutritionHeroImages[currentIndex].title[language]}
              </span>
            </motion.div>
          </div>
        </motion.div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex items-center gap-4">
        ...
        <div className="flex gap-2">
          {nutritionHeroImages.map((_, index) => (
            <button
              key={index}
...
const nutrientStories = [
  ...
  },
  {
    id: 5,
    icon: Heart,
    title: { fr: 'VITAMINES', en: 'VITAMINS' },
    color: 'from-rose-500/20 to-red-400/20',
    image: '/images/Alimentation/legumes-de-fruits-et-de-cereales.jpg',
    description: { 
      fr: 'Soutien immunitaire et vitalité optimale', 
      en: 'Immune support and optimal vitality' 
    }
  }
];
        ...
        >
          <div className="inline-flex items-center gap-3 px-8 py-4 rounded-2xl bg-gradient-to-r from-emerald-100 to-green-100 dark:from-emerald-900/40 dark:to-green-900/40 border-2 border-emerald-200 dark:border-emerald-800 backdrop-blur-sm shadow-xl">
            ...
            <span className="text-base font-semibold text-emerald-800 dark:text-emerald-300">
              {language === 'fr' 
                ? '5 nutriments essentiels • Alimentation équilibrée • Santé optimale' 
                : '5 essential nutrients • Balanced diet • Optimal health'
              }
            </span>
          </div>
        </motion.div>
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950">
      ...
      <section className="relative min-h-[70vh] flex items-center py-12 overflow-hidden">
        <NutritionHeroCarousel language={language} />
            ...
            {t.focus.map((area, idx) => (
              ...
              >
                <div className="flex items-center gap-4 mb-6">
                  <div className="p-3 rounded-full bg-emerald-100 dark:bg-emerald-900/30 group-hover:bg-emerald-200 dark:group-hover:bg-emerald-800/50 transition-colors">
